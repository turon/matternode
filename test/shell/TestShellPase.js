...
describe(path.basename(__filename), async function () {
    ...
    it("Test Shell: pase", async function () {
        try {

            node0.proc.send("pase -p "+node1.Port)
            await node0.proc.expect("PASE: Create Session with Secret!!!")
            await node1.proc.expect("PASE: Create Session with Secret!!!")
    ...
    it("Test Shell: pase help", async function () {
        node0.proc.send("pase --help")
